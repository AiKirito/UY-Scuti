function rebuild_rom {
	...
	while true; do
		echo -e "\n Please put the partition file to be flashed into the Ready-to-flash/images folder in the selected working domain directory"
		echo -e "\n [1] Fastboot(d) Rom " "[2] Odin Rom " "[Q] Unpack\n"

		# 检查是否有 img 文件
		if compgen -G "$WORK_DIR/$current_workspace/Repacked/*.img" >/dev/null; then
			echo -e " [M] Move easily\n"
		fi
	...
	if [[ "$main_choice" == "1" ]]; then
		...
		while true; do
			...
			if [[ "$device_model" == "q" ]]; then
				echo "  cancels packaging and returns to the work domain menu."
				return
